...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editTask) {
      try {
        const response = await apiClient.put(`/tasks/${editTask.id}`, {
          title: newTask,
        });
        setTasks(
  ...
  const handleComplete = async (taskId) => {
    try {
      ...
      setTasks(
        tasks.map((task) => (task.id === taskId ? updatedTask.data : task))
      );
    } catch (error) {
        ...
        <div className="col-12 col-md-8">
          ...
          <ul className="list-group mt-4">
            ...
            {[...incompleteTasks, ...completedTasks].map((task) => (
              <li
                key={task.id}
                className={`list-group-item d-flex justify-content-between align-items-center mb-3 ${task.completed ? "list-group-item-success" : ""
                  }`}
              >
                <div className="me-3">
                  <input
